...
-- data State = State {_operatorStack :: [OperatorStackType], _outputStack :: [C.Ast V.Value], _isCast :: Bool}
-- $(makeLenses 'State)
...
--     (a : b : rest) -> C.modify (outputStack .~ (C.Application pos (C.Application pos (C.Variable pos name) b) a : rest))
...
--       variable = foldr (C.Application pos) (C.Variable pos name) (reverse args)
--   C.modify (outputStack .~ variable : rest)
...
--       C.modify (outputStack %:~ C.Variable pos name)
--       incrementStackHead
...
--   C.modify (outputStack %:~ C.Value pos (V.Value r u symbol))
--   shuntingYard' False ts
...
--   C.modify (outputStack %:~ C.Value pos (mapper $ V.fromRational r))
--   incrementStackHead
--   shuntingYard' False ts
...
-- shuntingYard :: Bool -> [Token] -> C.Calculator a (C.Ast V.Value)
...
-- expression :: String -> C.Calculator () (C.Ast V.Value)
-- expression = mapTokenizer $ expr

-- definition :: String -> C.Calculator () (String -> C.Ast ())
...
--       variable = foldr (C.Lambda pos) (C.Variable pos name) args
--   return $ \source -> C.Definition pos source name variable
--   where
...
--   case maybeCast of
--     Nothing -> return ast
--     Just (pos, castValue) -> return $ C.Cast pos ast castValue

-- cast :: Tokenizer (E.Position, C.Ast V.Value)
-- cast = do
--   start <- getPosition
--   char '['
--   ts <- tokenize
